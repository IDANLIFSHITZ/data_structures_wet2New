...
/*
 * increase number of wins for team by change.
 */
...
olympics_t::olympics_t(): teamsTree(AVL<Team*, Pair<int,int>>([](const Team* team){if(team->getNumOfPlayers()== 0){return 0;}return team->getStrength();})){}
...
StatusType olympics_t::add_team(int teamId)
{
    ...
    try
    {
        ...
        return teamsTable.add(team, teamId);
    }
...
StatusType olympics_t::remove_team(int teamId)
{
    ...
    teamsTree.remove(Pair<int,int>(out.ans()->getStrength(), -teamId));
...
output_t<int> olympics_t::play_tournament(int lowPower, int highPower)
{
    ...
    int numOfSmallerLow = this->teamsTree.get_number_of_smaller_nodes_by_force(lowPower-1);
    int numOfSmallerHigh = this->teamsTree.get_number_of_smaller_nodes_by_force(highPower);
    int numOfTeams = numOfSmallerHigh - numOfSmallerLow;

    if (!is_power_of_2(numOfTeams)) //if number of teams in tournament is not a power of 2.
    {
        return StatusType::FAILURE;
    }
    ...
    {
        //get highest team from half weakest teams of the tournaments.
        ...
        //update win to half top teams in the tournaments.
        teamsTree.update_extra_in_range(lowTeamKey, highTeamKey, 1);
    }
    return highTeam->getId();
}
